...
class TwoDSpec:
    ...
    def get_rectification(
        ...
        if coeff is not None:
            ...
            s[0] -= min(s[0][one_fifth - 1 : -one_fifth])
            s[0] /= max(s[0][one_fifth - 1 : -one_fifth])
            s_down = []
            s_up = []
            ...
            for k in range(n_down):
                ...
                s_down[k] -= min(s_down[k][one_fifth - 1 : -one_fifth])
                s_down[k] /= max(s_down[k][one_fifth - 1 : -one_fifth])
            ...
            for k in range(n_up):
                ...
                s_up[k] -= min(s_up[k][one_fifth - 1 : -one_fifth])
                s_up[k] /= max(s_up[k][one_fifth - 1 : -one_fifth])
            ...
            for i in range(1, len(s_all)):
                ...
                corr = signal.correlate(
                    10.0 ** s_all[i][one_fifth - 1 : -one_fifth],
                    10.0 ** s_all[i - 1][one_fifth - 1 : -one_fifth],
                )
                shift_upsampled[i - 1 :] += (
                    spec_size_tmp
                    - 2 * one_fifth
    ...
    def _fit_sky(
        ...
        if (sky_width_dn > 0) or (sky_width_up > 0):
            ...
            self.logger.debug(
                "Background sky flux is %s.", count_sky_extraction_slice
            )
    ...
    def _optimal_extraction_horne86(
        ...
        if model == "gauss":
            ...
            self.logger.debug(
                "The correction factor of the gaussian profile is %s.",
                correction,
            )
    ...
    def _optimal_extraction_marsh89(
        ...
        if isinstance(apwidth, (float, int)):
            ...
            self.logger.error(
                "apwidth can only be an int or a list of two ints. It is set "
                "to the default value to continue the extraction."
            )
            width_dn = 7
            width_up = 7
        ...
        if np.any(bad_residual_frame_mask.nonzero()):
            self.logger.warning(
                "Found bad residual_frame values at: %s",
        ...
        while new_bad_pixels:
            i += 1
            self.logger.debug("Beginning iteration %s.", i)
            ...
            inv_e_variance_subset = inv_e_variance[
                q_cols[0] : q_cols[1] + 1, :
            ]
            ...
            self.logger.info(
                "Rejected %s pixels in this iteration.", number_rejected
            )
