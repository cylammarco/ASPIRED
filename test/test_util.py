...
def test_cutoff_mask():
    cutoff_mask = util.create_cutoff_mask(dummy_image)
...
def test_cutoff_mask_list():
    cutoff_mask = util.create_cutoff_mask(dummy_image, cutoff=[0, 1000])
    healed_image = util.bfixpix(dummy_image, cutoff_mask, retdat=True)
    assert np.sum(healed_image) == np.size(dummy_image)
...
def test_cutoff_mask_does_nothing():
    cutoff_mask = util.create_cutoff_mask(dummy_image, cutoff=65500)
    healed_image = util.bfixpix(dummy_image, cutoff_mask, retdat=True)
...
def test_cutoff_mask_grow():
    cutoff_mask = util.create_cutoff_mask(dummy_image, grow=True)
    healed_image = util.bfixpix(dummy_image, cutoff_mask, retdat=True)
    assert np.sum(healed_image) == np.size(dummy_image)
...
def test_cutoff_mask_does_nothing_grow():
    cutoff_mask = util.create_cutoff_mask(dummy_image, cutoff=65500, grow=True)
    healed_image = util.bfixpix(dummy_image, cutoff_mask, retdat=True)
    assert np.sum(healed_image) == dummy_sum
...
def test_bad_mask():
    bad_mask = util.create_bad_mask(dummy_image_nan)
...
def test_bad_mask_list():
    bad_mask = util.create_bad_mask(dummy_image_none)
    healed_image = util.bfixpix(dummy_image, bad_mask, retdat=True)
...
def test_bad_mask_does_nothing():
    bad_mask = util.create_bad_mask(dummy_image)
    healed_image = util.bfixpix(dummy_image, bad_mask, retdat=True)
    assert np.sum(healed_image) == dummy_sum


def test_bad_mask_1D_array():
    util.create_bad_mask(np.ones(100))


# Repeat with grow set to True
# Healing the diagonal of pixels with NAN
def test_bad_mask_grow():
    bad_mask = util.create_bad_mask(dummy_image_nan, grow=True, diagonal=True)
    healed_image = util.bfixpix(dummy_image, bad_mask, retdat=True)
    assert np.sum(healed_image) == np.size(dummy_image)
...
def test_bad_mask_list_grow():
    bad_mask = util.create_bad_mask(dummy_image_none, grow=True, diagonal=True)
    healed_image = util.bfixpix(dummy_image, bad_mask, retdat=True)
    assert np.sum(healed_image) == np.size(dummy_image)
...
def test_bad_mask_does_nothing_grow():
    bad_mask = util.create_bad_mask(dummy_image, grow=True, diagonal=True)
    healed_image = util.bfixpix(dummy_image, bad_mask, retdat=True)
    assert np.sum(healed_image) == dummy_sum
