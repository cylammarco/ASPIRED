...
lines_ref = lines_ref_init_resampled - np.percentile(lines_ref_init_resampled, 5)
...
spec_wave = np.arange(len(data_split)) * w_size + w_size / 2.

# Looping through pixels larger than middle pixel 
for i in range(w_iter//2+1, len(data_split)):
    spec[:,i] = (spec[:,i-1] * scaling * int(N_resample*scale_solution[i]) / N_resample - shift_solution[i]) / scaling
    spec_val[:,i] = signal.resample(np.nanmedian(data_split[i], axis=Saxis), int(N_resample*scale_solution[i]))[(spec[:,i] * scale_solution[i]).astype('int')]

# Looping through pixels smaller than middle pixel
for i in range(w_iter//2, -1, -1):
    spec[:,i] = (spec[:,i+1] * scaling + shift_solution[i+1]) / (int(N_resample * scale_solution[i+1]) / N_resample) / scaling
    spec_val[:,i] = signal.resample(np.nanmedian(data_split[i], axis=Saxis), int(N_resample*scale_solution[i]))[(spec[:,i] * scale_solution[i]).astype('int')]
...
if Saxis is 1:
    ...
    imshow(np.log10(np.transpose(data)), vmin=vmin, vmax=vmax, aspect='auto', origin='lower')
...
scatter(np.ones(len(spec))*spec_wave[w_iter//2], spec[:,w_iter//2], zorder=20, s=10, color='red')
